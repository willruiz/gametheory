...
def gen_entry_buttons(parent_in, root, canvas_in):
    ...
    quit_btn  = tk.Button(root, text = "Exit" , bg = cd.lite_ornge, command = lambda: cb.quit_game(parent_in), width = 5*int(parent_in.unit_width/20), height = 3)
    
    canvas_in.create_window(parent_in.cenv - int(parent_in.unit_height/2), parent_in.bot + 1 *(parent_in.unit_height/4), window=print_btn)
    canvas_in.create_window(parent_in.cenv - int(parent_in.unit_height/2), parent_in.bot + 2 *(parent_in.unit_height/4), window=save_btn)
    ...
    canvas_in.create_window(parent_in.cenv + int(parent_in.unit_height/2), parent_in.bot + 2 *(parent_in.unit_height/4), window=load_btn)
    canvas_in.create_window(parent_in.cenv, parent_in.bot + 1 *int(parent_in.unit_height/4), window=nfg_btn)
    canvas_in.create_window(parent_in.cenv, parent_in.bot + 2 *int(parent_in.unit_height/4), window=rnfg_btn)

    canvas_in.create_window(parent_in.cenv, parent_in.bot + 3.5 *int(parent_in.unit_height/4), window=quit_btn)
